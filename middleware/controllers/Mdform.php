...
//        }
    ...
    public function jsonDataInsertDelete($reloationRow) {
        ...
        if ($reloationRow['recordId'] && $reloationRow['data']) {
            ...
            if ($tblName) {
                ...
                if ($isTblCreated) {
                     ...
                     * Tur nemsn
                     */
                    $this->db->Execute('DELETE FROM '.$tblName.' WHERE '.$reloationRow['clear']['column'].' IS NULL'); 
                }
            }
                ...
                foreach ($reloationRow['clear']['dtl'] as $crow) {
                    ...
                    if ($crow['tableName'] && $dtlId) {
                        if ($instanceExp->model->table_exists($this->db, $crow['tableName'])) {
                             ...
                             * Tur nemsn
                             */                            
                            $this->db->Execute('DELETE FROM '.$crow['tableName'].' WHERE '.$crow['column'].' IS NULL'); 
                        }
                    }
                }            
            }            
        ...
        if (isset($reloationRow['endToEndLog'])) {
            ...
            $this->db->Execute("DELETE FROM END_TO_END_SESSION_LOG WHERE SRC_DATASET_ID = $deleteIdPh AND SRC_RECORD_ID = ".$this->db->Param(1), [$endToEndLogDatasetId, $endToEndLogRecordId]);
        ...
        if (isset($reloationRow['wfmLog'])) {
            ...
            $this->db->Execute("DELETE FROM META_WFM_LOG WHERE REF_STRUCTURE_ID = $deleteIdPh AND RECORD_ID = ".$this->db->Param(1), [$refStructureId, $recordId]);
        ...
        if (isset($reloationRow['ecmContent']['header'])) {
            ...
            $this->db->Execute("DELETE FROM ECM_CONTENT WHERE CONTENT_ID IN (SELECT CONTENT_ID FROM ECM_CONTENT_MAP WHERE REF_STRUCTURE_ID = $deleteIdPh AND RECORD_ID = ".$this->db->Param(1).")", [$refStructureId, $recordId]);
            $this->db->Execute("DELETE FROM ECM_CONTENT_MAP WHERE REF_STRUCTURE_ID = $deleteIdPh AND RECORD_ID = ".$this->db->Param(1), [$refStructureId, $recordId]);
    ...
    public function jsonDataDmRecordMapInsertDelete($mapData, $mapRowKey) {
        ...
        try {
            ...
            $this->db->Execute('DELETE FROM META_DM_RECORD_MAP WHERE SRC_REF_STRUCTURE_ID = '.$deleteIdPh.' AND TRG_REF_STRUCTURE_ID = '.$deleteIdPh1.' AND SRC_RECORD_ID = '.$deleteIdPh2, [$mapData['SRC_REF_STRUCTURE_ID'], $mapData['TRG_REF_STRUCTURE_ID'], $mapData['SRC_RECORD_ID']]); 
