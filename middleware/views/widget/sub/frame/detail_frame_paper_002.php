...
?>
<div class="bpdtl-widget-detail_frame_paper_002 d-flex">
    <?php 
    ...
    ?>
    <div class="bpdtl-widget-detail_frame_paper_002-body">
        {content}
    </div>
</div>
...
.bpdtl-widget-detail_frame_paper_002 {
...
.bpdtl-widget-detail_frame_paper_002-body table.kpi-dtl-table > tbody > tr > td:first-of-type, 
.bpdtl-widget-detail_frame_paper_002-body table.kpi-dtl-table > tbody > tr > td:first-of-type {
    width: 30px;
    min-width: 30px;
}
.bpdtl-widget-detail_frame_paper_002-body table.kpi-dtl-table > tbody > tr > td.bp-dtl-rownumber,
.bpdtl-widget-detail_frame_paper_002-body table.kpi-dtl-table > tbody > tr > td:last-child {
    display: none;
}
.bpdtl-widget-detail_frame_paper_002-body .bp-overflow-xy-auto {
    border: none !important;
}
.bpdtl-widget-detail_frame_paper_002-body table.kpi-dtl-table>tbody>tr.currentTarget>td {
    border-bottom: none !important;
}
.bpdtl-widget-detail_frame_paper_002-body .table-bordered td {
    border: none !important;
}
.bpdtl-widget-detail_frame_paper_002-body .table-bordered {
    border: none !important;
}
.bpdtl-widget-detail_frame_paper_002-body .table-hover tbody tr:hover {
    color: inherit;
    background-color: inherit; 
}
.bpdtl-widget-detail_frame_paper_002-body table.bprocess-theme1.table-hover>tbody>tr:hover>td {
    background-color: inherit;
}
div[data-process-id="<?php echo $this->methodId; ?>"] .bpdtl-widget-detail_frame_paper_002 .jstree-ocl {
    display: none;
}
div[data-process-id="<?php echo $this->methodId; ?>"] .bpdtl-widget-detail_frame_paper_002 .jstree-anchor {
    display: block;
...
<script type="text/javascript">
    ...
    $('#bp-window-<?php echo $this->methodId; ?>').on('click', '.bpPaper001AddMainRow_<?php echo $this->methodId; ?>', function(e) {
        ...
        var $getTable = $parent.find('.kpi-dtl-table:eq(0)');
            ...
            success: function (dataStr) {
                var $html = $('<div />', {html: dataStr});
                    ...
                    if (paragraphNumberArr[0]) {
                        ...
                        if (editJsonSavedData.length) {           
                            widget_detail_frame_paper_002_numbering_fromexp(ppp, conContractTemplateParentId, isSubRow);
                        }
                    }
                    addNewRowIndexPaper001++;
                    
                    if ($this.closest('.kpi-dtl-table').hasClass('cool-row')) {
    ...
    function initJstree_widget_detail_frame_paper_002(data) {
        ...
        if ($jsTreeFilter) {   
            $jsTreeFilter.jstree({
                ...
                var elem = $('.bpdtl-widget-detail_frame_paper_002-body').find('div[data-bp-detail-container]');
...
//                }
                var $table = $('.bpdtl-widget-detail_frame_paper_002-body').find('table.kpi-dtl-table'), 
                ...
                setTimeout(function() {
                    ...
                    widget_detail_frame_paper_002_numbering($jstreeAnchor);
    ...
    function widget_detail_frame_paper_002_numbering_fromexp(number, parentId, isSubRow) {
        var $table = $('.bpdtl-widget-detail_frame_paper_002-body').find('table.kpi-dtl-table'), 
            $tbody = $table.find('.tbody:eq(0)'), 
            $rows = $tbody.find('.bp-detail-row');
    ...
    function widget_detail_frame_paper_002_numbering_fromexp_add(number, parentId, number2) {
        var $table = $('.bpdtl-widget-detail_frame_paper_002-body').find('table.kpi-dtl-table'), 
            $tbody = $table.find('.tbody:eq(0)'), 
            $rows = $tbody.find('.bp-detail-row');
    ...
    function widget_detail_frame_paper_002_numbering($jstreeAnchor) {
        if ($jstreeAnchor.length) {
            ...
            for (i; i < len; i++) { 
                ...
                if (i) {
                    ...
                    if ($numberingColumn) {
                    ?>
                    widget_detail_frame_paper_002_setNumbering($subElement, n);
                    <?php
                    }
                    ?>
                }
    ...
    function widget_detail_frame_paper_002_setNumbering($element, n) {
        var $table = $('.bpdtl-widget-detail_frame_paper_002-body').find('table.kpi-dtl-table'), 
            $tbody = $table.find('.tbody:eq(0)'), 
            $rows = $tbody.find('.bp-detail-row'),
...
//        }        
    ...
    function search_widget_detail_frame_paper_002_treeview(valueInput) {
        var $table = $('.bpdtl-widget-detail_frame_paper_002-body').find('table.kpi-dtl-table'), 
            $tbody = $table.find('tbody:eq(0)');
    ...
    function detail_frame_paper_002_basket_function(rows) {
        if (rows) {
            ...
            var resultTree = resolveParentChild_detail_frame_paper_002(rowsWithChild, 'basket');  
            
            // if (Object.keys(editJsonSavedData).length) {
                editJsonData = editJsonData.concat(resultTree);
                initJstree_widget_detail_frame_paper_002(editJsonData);
            // } else {                        
            //     initJstree_widget_detail_frame_paper_002(resultTree);
            // }
        }
        
        return true;
    }
    function resolveParentChild_detail_frame_paper_002(rows, basket) {
            ...
            if (rows[i].hasOwnProperty(_parentId) && (rows[i][_parentId] == null || rows[i][_parentId] == '')) { 
                if (Object.keys(editJsonSavedData).length && basket !== 'basket') {         
                    data[n] = {
                        ...
                        'rowdata': rows[i], 
                        'childdata': resolveParentChild2_detail_frame_paper_002(rows, rows[i]['<?php echo Str::lower($filterColumn); ?>'])
                    };        
                    n++;                                
                } else {
                    ...
                    if (!isExist) {
                        data[n] = {
                            ...
                            'text': gridHtmlDecode(rows[i]['name']).replace(/<\/?[^>]+(>|$)/g, ""), 
                            'rowdata': rows[i], 
                            'childdata': resolveParentChild2_detail_frame_paper_002(rows, rows[i]['id'])
                        };
                        n++;
                    }
                }
                /*
                data[n]['children'] = resolveParentChild2_detail_frame_paper_002(rows, rows[i]['id']);
                */                
            }
        }    
    ...
    function resolveParentChild2_detail_frame_paper_002(rows, parentid) {
        var data = [], n = 0;
    ...
    function widget_detail_frame_paper_002_remove_treeview() {
