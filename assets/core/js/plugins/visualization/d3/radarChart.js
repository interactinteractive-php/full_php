function RadarChart(id, data, options) {
    ...
    var axisName = cfg["axisName"], areaName = cfg["areaName"], value = cfg["value"];
    ...
    blobWrapper
        ...
        .attr("class", function(d) {
                return "radarArea" + " " + d[0][areaName].replace(/\s+/g, '').replace(/\(/g, '').replace(/\)/g, '').replace(/:/g, '').replace(/\./g, '').replace(/&/g, ''); //Remove spaces from the areaName string to make one valid class name
        })
        ...
        .on('mouseout', function(){
            ...
            d3.selectAll(".radarArea")
                .transition().duration(200)
                .style("fill-opacity", cfg.opacityArea);
        }).append("svg:title").text(function(d, i) { return d[0][areaName]; });
    ...
    function cellover(d) {
        ...
        d3.selectAll(".radarArea")
                ...
                .style("fill-opacity", 0.1); 
        //Bring back the hovered over blob
        d3.select("." + data[d][0][areaName].replace(/\s+/g, '').replace(/\(/g, '').replace(/\)/g, '').replace(/:/g, '').replace(/\./g, '').replace(/&/g, ''))
                .transition().duration(200)
                .style("fill-opacity", 0.7);	
    }
    ...
    .on("cellclick", function(d) { 
        ...
        var $this = $(this);
        var thisClassName = data[d][0][areaName].replace(/\s+/g, '').replace(/\(/g, '').replace(/\)/g, '').replace(/:/g, '').replace(/\./g, '').replace(/&/g, '');
        var radarPart = d3.select('.' + thisClassName);
        var parentRadarPart = $(radarPart.node().parentNode);
        
        if (parentRadarPart.hasClass('d-none')) {
            parentRadarPart.removeClass('d-none');
            $this.find('text.label').css('fill', '');
        } else {
            parentRadarPart.addClass('d-none');
            $this.find('text.label').css('fill', '#bababa');
        }
    });
