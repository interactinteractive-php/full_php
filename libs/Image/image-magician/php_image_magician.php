...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
    ...
    #
    {
        ...
        if ($this->width < $textWidth) {
            ...
            $widthDiff = $textWidth - $this->width + 3;
            $img = imagecreatetruecolor($widthDiff, $this->height);
            imagesavealpha($img, true);
            $color = imagecolorallocatealpha($img, 0, 0, 0, 127);
            imagefill($img, 0, 0, $color);
            
            $merged_image = imagecreatetruecolor($this->width + $widthDiff, $this->height);
            
            imagealphablending($merged_image, false);
            imagesavealpha($merged_image, true);
            
            imagecopy($merged_image, $this->imageResized, 0, 0, 0, 0, $this->width, $this->height);
            imagecopy($merged_image, $img, $this->width, 0, 0, 0, $widthDiff, $this->height);
            
            $this->imageResized = $merged_image;
