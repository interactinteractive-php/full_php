    ...
    Suggest.prototype = {
        ...
        __filterData: function(q, data) {
            ...
            this.$items.filter(function (index) {
				...
				if (options.filter.firstdottedafter) {
                    value = value.substring(value.indexOf('.')).replace('.', '');
                }
				
				if (options.filter.beginfilterchar) {
					value = value.substring(options.filter.beginfilterchar, 100);
				}
				
				if (options.filter.filterrule == 'startsWith') {
					var isFiltered = value.startsWith(q);
				} else {
					var isFiltered = (value.indexOf(q) != -1);
				}
                return isFiltered;
